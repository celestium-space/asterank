...
NEAT_FIELDS_SMALL = ['obs_id', 'ra', 'dec',
                     'time', 'exposure', 'triplet', 'key']
...
def search_ephem(epoch, ecc, per, per_date, om, w, inc, h):
    ...
    if cached:
        ...
        r = requests.get(SEARCH_EPHEM_URL %
                         (epoch, ecc, per, per_date, om, w, inc, h))
        results = parse_results_table(r.text, NEAT_FIELDS_LARGE)
        redis.set(redis_key, json.dumps(results))
        return results
...
def get_image_info(key):
    ...
    result = {'success': True, 'url': urllib.parse.urljoin(
        URL_BASE, matches.group(1))}
