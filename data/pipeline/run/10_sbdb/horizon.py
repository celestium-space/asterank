...
def _run(partial=False):
    ...
    # necessary for upcoming pass lookups
    ...
    for row in reader:
        ...
        if 'pdes' in row and 'prov_des' not in row:
            # backwards compatibility for NASA change
            row['prov_des'] = row['pdes']
        if m:
            # Set delta-v first
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Load data from NASA/JPL SBDB')
    parser.add_argument(
        '--data_path', help='path to sbdb export', default='data/latest_sbdb.csv')
    parser.add_argument(
        '--dv_path', help='path to delta-v calculations', default='data/deltav/db.csv')
    parser.add_argument(
        '--mass_path', help='path to mass data', default='data/masses.txt')
    parser.add_argument(
        'fn', choices=['populateDb', 'populatePartialDb', 'compositions', 'materials'])
