...
@app.route('/asterank/sitemap.xml')
@app.route('/asterank/robots.txt')
...
@app.route('/asterank/upcoming')
def upcoming():
    return render_template('upcoming.html')


@app.route('/asterank/3d/')
def view_3d_slash():
    return render_template('full3d.html', noop=noop_filter)


@app.route('/asterank/offline_3d')
def view_3d_offline():
    pt_vars = {}
    pt_vars['offline_mode'] = True
    pt_vars['score_rankings'] = json.dumps(
        api.rankings('score', 4000, True), allow_nan=False)
    pt_vars['value_rankings'] = json.dumps(
        api.rankings('value', 4000, True), allow_nan=False)
    pt_vars['accessibility_rankings'] = json.dumps(
        api.rankings('accessibility', 4000, True), allow_nan=False)
    pt_vars['smallest_rankings'] = json.dumps(
        api.rankings('smallest', 4000, True), allow_nan=False)

    return render_template('full3d.html', noop=noop_filter, passthrough_vars=pt_vars,
                           offline_mode=True)


@app.route('/asterank/3d/notsupported.html')
def notsupported_3d():
    return render_template('notsupported.html')


@app.route('/asterank/asteroid-<asteroid_slug>')
...
@app.route('/asterank/api/exoplanets')
def api_exoplanets():
    try:
        query = json.loads(request.args.get('query'))
        limit = min(1000, int(request.args.get('limit')))
...
@app.route('/asterank/api/asterank')
def api_asterank():
    try:
        query = json.loads(request.args.get('query'))
        limit = min(1000, int(request.args.get('limit')))
...
@app.route('/asterank/api/rankings')
def rankings():
    try:
        limit = int(request.args.get('limit')) or 10
        orbital_info_only = request.args.get('orbits_only')
        results = api.rankings(request.args.get(
            'sort_by'), limit, orbits_only=orbital_info_only)
...
@app.route('/asterank/api/autocomplete')
def autocomplete():
...
@app.route('/asterank/api/compositions')
def compositions():
    json_resp = json.dumps(api.compositions())
    return Response(json_resp, mimetype='application/json')


@app.route('/asterank/jpl/lookup')
def horizons():
...
@app.route('/asterank/api/skymorph/search_position')
...
@app.route('/asterank/api/skymorph/image')
def skymorph_image():
...
@app.route('/asterank/api/skymorph/fast_image')
...
@app.route('/asterank/api/sdss/get_unknown_group')
...
@app.route('/asterank/api/sdss/image')
def sdss_image():
    from sdss import sdss
...
@app.route('/asterank/api/stackblink/get_neat_control_group')
...
@app.route('/asterank/api/stackblink/get_sdss_unknown_group')
...
@app.route('/asterank/api/stackblink/record', methods=['GET', 'POST'])
...
@app.route('/asterank/exoplanets')
@app.route('/asterank/kepler3d')
def kepler3d():
...
@app.route('/asterank/api/user_objects', methods=['GET', 'POST'])
def user_objects():
    if request.method == 'GET':
        # limit set to 300 objects for now
        return jsonify({'results': api.retrieve_user_objects(300)})
...
@app.route('/asterank/about', methods=['GET', 'POST'])
def about():
    if request.method == 'GET':
...
@app.route('/asterank/feedback')
@app.route('/asterank/contact')
def contact():
    return render_template('contact.html')


@app.route('/asterank/mpc')
def mpc():
    return render_template('mpc.html')


@app.route('/asterank/kepler')
def kepler():
...
@app.route('/asterank/neat')
def neat_docs():
    return redirect('/skymorph')


@app.route('/asterank/skymorph')
def skymorph_docs():
    return render_template('skymorph.html')


@app.route('/asterank/api')
def api_route():
