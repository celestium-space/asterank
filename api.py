...
VALID_SORTS = set(
    ["price", "profit", "closeness", "score", UPCOMING_SORT, SMALL_SIZE_SORT, MOID_SORT]
)
...
def rankings(sort_by, limit, orbits_only=False):
    ...
    if sort_by == MOID_SORT:
        ...
        print(f"TEST: {sort_by}")
        results = list(
            asteroids.find({"state": "available"}, fields)
...
def autocomplete(query, limit):
    ...
    ret = list(
        asteroids.find(
            {"full_name": {"$regex": regx}, "state": "available"}, {"_id": False}
        )
    )
    # .limit(limit))
...
def upcoming_passes():
    jpl_objs = (
        jpl.find(
            {
                "Next Pass": {"$exists": True, "$ne": None},
                "Next Pass.date_iso": {"$gte": datetime.datetime.now().isoformat()},
            },
            {"_id": False},
        )
        .sort("Next Pass.date_iso", direction=pymongo.ASCENDING)
        .limit(30)
    )
    ...
    for result in jpl_objs:
        if result["tag_name"] in seen:
            continue
        roid = asteroids.find_one(
            {"prov_des": result["tag_name"], "state": "available"}, {"_id": False}
        )
...
def ranking_by_smallest(limit, fields):
    return list(
        asteroids.find({"diameter": {"$ne": ""}, "state": "available"}, fields)
        .sort("diameter", direction=pymongo.ASCENDING)
        .limit(limit)
    )
...
def ranking_by_moid(limit, fields):
    return list(
        asteroids.find({"moid": {"$ne": ""}, "state": "available"}, fields)
        .sort("moid", direction=pymongo.ASCENDING)
        .limit(limit)
    )
...
def jpl_lookup(query):
    ...
    if not result:
        print(f"JPL lookup: {query} not found in cache...")
        # maybe it's not cached; try querying for it from horizons
        try:
            a = JPL_Asteroid(query)
            a.load()
            print(f"JPL lookup: {query} loaded from JPL")
            result = a.data
            result["tag_name"] = query
            jpl.insert(result)  # cache
            del result["_id"]
        except:
            print(f"JPL lookup: {query} lookup failed")
            return None
    else:
        print(f"JPL lookup: {query} found in cache")
...
def exoplanets(query, limit):
    ...
    results = list(
        exoplanets_coll.find(query, {"_id": False}).sort(
            "koi_sma", direction=pymongo.DESCENDING
        )
    )
    ...
    for result in results:
        if (
            result["koi_disposition"] != "FALSE POSITIVE"
        ...
        # appendme['a'] *= 20
        # http://galacticfool.com/orbital-distance-and-orbital-period/
